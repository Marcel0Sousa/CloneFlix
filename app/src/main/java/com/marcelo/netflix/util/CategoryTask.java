...
public class CategoryTask extends AsyncTask<String, Void, List<Category>> {
    ...
    private CategoryLoader setCategoryLoader;
    ...
    protected List<Category> doInBackground(String... params) {
        ...
        try {
            ...
            if (responseCode > 400) {
                ...
                throw new IOException("Erro na comunicação com servidor");
    ...
    private List<Category> getCategories(JSONObject json) throws JSONException {
        ...
        for (int i = 0; i < categoryArray.length(); i++) {
           ...
           for (int j = 0; j < movieArray.length(); j++){
               ...
               String coberUrl = movie.getString("cover_url");
               int id = movie.getInt("id");

              Movie movieObj = new Movie();
              movieObj.setCoverUrl(coberUrl);
              movieObj.setId(id);
    ...
    protected void onPostExecute(List<Category> categories) {
        ...
        // listener
        if (setCategoryLoader != null) {
            setCategoryLoader.onResult(categories);
        }
    }
